...
const TeamPage = () => {
  ...
  // Board members organized by rows (as per user's layout)
  const boardMembersByRow = [
    // Row 1
    [
      {
  ...
  // Flattened array for modal functionality
  const allBoardMembers = boardMembersByRow.flat();
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-col items-center gap-8">
                {boardMembersByRow.map((row, rowIdx) => {
                  // Calculate the starting index for this row
                  const rowStartIndex = boardMembersByRow.slice(0, rowIdx).reduce((sum, r) => sum + r.length, 0);
                  // Use grid for rows with multiple items, flex with center for single items
                  const isMultiItemRow = row.length > 1;
                  return (
                    <div 
                      key={rowIdx}
                      className={isMultiItemRow 
                        ? "grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl mx-auto" 
                        : "flex justify-center gap-8"
                      }
                    >
                      {row.map((member, memberIdx) => {
                        const currentIndex = rowStartIndex + memberIdx;
                        return (
                          <div 
                            key={`${rowIdx}-${memberIdx}`}
                          ...
                          >
                            <div className="relative w-32 h-32 mx-auto mb-4 rounded-full overflow-hidden bg-gray-100">
                              <Image
                                ...
                                className="object-cover"
                                sizes="128px"
                                loading={currentIndex < 4 ? undefined : 'lazy'}
